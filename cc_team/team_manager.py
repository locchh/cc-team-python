...
class TeamManager:
    ...
    def _parse_agent_config(self, agent_dir: Path) -> Optional[AgentConfig]:
        ...
        try:
            ...
            host = agent_config_data.get('host', settings.get('default_host', 'localhost'))
            ...
            description = agent_config_data.get('description', f'A2A agent: {agent_name}')
    ...
    def validate_team(self) -> List[str]:
        ...
        for agent_name, agent_config in self.agents.items():
            if agent_config.port in used_ports:
                issues.append(f"Port conflict: {agent_name} uses port {agent_config.port}")
